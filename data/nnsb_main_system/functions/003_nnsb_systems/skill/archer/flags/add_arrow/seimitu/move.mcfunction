#tpでやる(ブロックの角だと貫通できるけど仕方ないよね)
execute as @s at @s unless block ~ ~ ~ #nnsb_main_system:can_live run kill @s
execute as @s at @s positioned ~-0.3 ~-0.3 ~-0.3 if entity @e[type=#nnsb_main_system:scope,dx=0.6,dy=0.6,dz=0.6] run function nnsb_main_system:003_nnsb_systems/skill/archer/flags/add_arrow/seimitu/attack
